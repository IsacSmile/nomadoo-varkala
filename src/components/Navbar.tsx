...
export const Navbar: React.FC<NavbarProps> = ({ onBookClick }) => {
  ...
  return (
    <header className="fixed top-0 left-0 right-0 z-50 transition-all duration-300 px-3 sm:px-6 pt-3 sm:pt-4 pointer-events-none">
      ...
      <div
        className={`max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 transition-all duration-300 rounded-2xl sm:rounded-3xl pointer-events-auto ${
          isScrolled
            ? 'bg-sand-50/95 backdrop-blur-md shadow-xl border border-sand-200/90 py-3'
            : 'bg-slate-950/85 backdrop-blur-md text-white py-3 border border-white/15 shadow-2xl'
        }`}
      >
        <div className="flex items-center justify-between">
          ...
          >
            <div className="relative overflow-hidden rounded-xl p-0.5 bg-white shadow-md border border-mangrove-100 group-hover:scale-105 transition-transform">
              <img
                ...
                className="h-9 w-auto max-w-[130px] sm:h-11 object-contain rounded-lg"
                onError={(e) => {
            ...
            <div className="flex flex-col">
              <span className={`text-base sm:text-lg font-bold tracking-tight leading-tight ${isScrolled ? 'text-mangrove-950' : 'text-white'}`}>
                NOMADOO <span className="text-sunset-500 font-extrabold">VARKALA</span>
              </span>
              <span className={`text-[10px] tracking-wider uppercase font-semibold ${isScrolled ? 'text-mangrove-800' : 'text-sand-300'}`}>
          ...
          <div className="hidden sm:flex items-center gap-3">
            ...
            <button
              onClick={onBookClick}
              className="flex items-center gap-2 bg-mangrove-800 hover:bg-mangrove-900 text-white text-xs sm:text-sm font-extrabold px-4 py-2.5 rounded-xl shadow-md hover:shadow-lg transition-all transform hover:-translate-y-0.5 active:translate-y-0"
            >
